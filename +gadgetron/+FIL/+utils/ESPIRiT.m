function [varargout] = ESPIRiT(cmd, varargin)
% BART	Call ESPIRiT command from Matlab.
%   [A, B] = ESPIRiT('command', X, Y) call command with inputs X Y and outputs A B
%
...
if nargin==0 || all(cmd==0)
    disp('Usage: ESPIRiT <command> <arguments...>');
    return
end
...
if isempty(bart_path)
    if exist('/usr/local/bart/bin/bart', 'file')
        bart_path = '/usr/local/bart/bin';
...
for i=1:nargin - 1
    ...
    gadgetron.FIL.utils.writecfl(in{i}, varargin{i});
end
...
if ispc
    ...
    cmd = [bart_path, '/bart ', cmd, ' ', in_str, ' ', out_str];
    fprintf([cmd '\n']);
    ERR = system(cmd);
end
...
for i=1:nargout
    if ERR==0
        varargout{i} = gadgetron.FIL.utils.readcfl(out{i});
    end
